...
#include "/etc/mujoco-2.2.1/sample/uitools.h"

#include "/etc/mujoco-2.2.1/sample/array_safety.h"
...
{
    d->ctrl[0]= -5000*d->qpos[0]            -100*d->qvel[0];    //shoulder_pan_joint
    d->ctrl[1]= -5000*(d->qpos[1]-(-1.5))   -250*d->qvel[1];    //shoulder_lift_joint
    d->ctrl[2]= -10000*d->qpos[2]           -50*d->qvel[2];     //elbow_joint
    d->ctrl[3]= -1500*d->qpos[3]            -10*d->qvel[3];     //wrist_1_joint
    d->ctrl[4]= -500*d->qpos[4]             -10*d->qvel[4];     //wrist_2_joint
    d->ctrl[5]= -1000*d->qpos[5]            -10*d->qvel[5];     //wrist_3_joint
    d->ctrl[6]= -60*d->qpos[6]              -20*d->qvel[6];     //right_hand_Thumb_Opposition
    d->ctrl[7]= -60*d->qpos[7]              -27*d->qvel[7];     //right_hand_Thumb_Flexion
    d->ctrl[8]= -80*d->qpos[8]              -27*d->qvel[8];     //right_hand_Index_Finger_Proximal
    d->ctrl[9]= -80*d->qpos[9]              -20*d->qvel[9];     //right_hand_Index_Finger_Distal
    d->ctrl[10]=-70*d->qpos[10]             -20*d->qvel[10];    //right_hand_Middle_Finger_Distal
    d->ctrl[11]=-70*d->qpos[11]             -25*d->qvel[11];    //right_hand_Middle_Finger_Proximal
    d->ctrl[12]=-100*d->qpos[12]            -30*d->qvel[12];    //right_hand_Finger_Spread
    d->ctrl[13]=-70*(d->qpos[19] - 0.1 )          +23*d->qvel[13];    //right_hand_Pinky
    d->ctrl[14]=-70*d->qpos[14]             -23*d->qvel[14];    //right_hand_Ring_Finger
